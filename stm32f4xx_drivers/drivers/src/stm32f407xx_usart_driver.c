#include "stm32f407xx_usart_driver.h"
 void USART_Init_ModeConfig(USART_Handle_t USARTHandle);
 void USART_Init_ParityConfig(USART_Handle_t USARTHandle);
 void USART_Init_HWFlowCtrlConfig(USART_Handle_t USARTHandle);
/*********************************************************************
...
void USART_SendData(USART_Handle_t USARTHandle, uint8_t *pTxBuffer,
	...
	for (uint32_t i = 0; i < Len; i++) {
		...
		if (USARTHandle.USART_Config.USART_WordLength == USART_WORDLEN_9BITS) {
			...
			USARTx_ptr(USARTHandle.USARTx)->DR =GET_KBITS(*pdata,9,0);// (*pdata & (uint16_t) 0x01FF);
			...
			USARTx_ptr(USARTHandle.USARTx)->DR =GET_KBITS(*pTxBuffer,8,0);// (*pTxBuffer & (uint8_t) 0xFF);
	...
	for (uint32_t i = 0; i < Len; i++) {
		...
		if (USARTHandle.USART_Config.USART_WordLength == USART_WORDLEN_9BITS) {
			...
			if (USARTHandle.USART_Config.USART_ParityControl
				...
				*((uint16_t*) pRxBuffer) = GET_KBITS(USARTx_ptr(USARTHandle.USARTx)->DR,9,0);//& (uint16_t) 0x01FF);
				...
				*pRxBuffer = GET_KBITS(USARTx_ptr(USARTHandle.USARTx)->DR,9,0);//& (uint8_t) 0xFF);
				pRxBuffer++;
			}
		} else {
			...
			if (USARTHandle.USART_Config.USART_ParityControl
				...
				//read 8 bits from DR
				*pRxBuffer = (uint8_t) GET_KBITS(USARTx_ptr(USARTHandle.USARTx)->DR,9,0);//& (uint8_t) 0xFF);
			}
				...
				*pRxBuffer = (uint8_t) GET_KBITS(USARTx_ptr(USARTHandle.USARTx)->DR,7,0);//& (uint8_t) 0x7F);
...
void USART_ClearFlag(uint8_t USARTx, uint16_t StatusFlagName) {
	 CLR_BIT_N(USARTx_ptr(USARTx)->SR,StatusFlagName);
 ...
 */
 void USART_PE_EV(USART_Handle_t USARTHandle) {

}
 void USART_FE_EV(USART_Handle_t USARTHandle) {
	/*
 ...
 void USART_NF_EV(USART_Handle_t USARTHandle) {
	/*
 ...
 void USART_ORE_EV(USART_Handle_t USARTHandle) {
 ...
 void USART_IDLE_EV(USART_Handle_t USARTHandle) {
 ...
 void USART_RXNE_EV(USART_Handle_t USARTHandle) {
	if (USARTHandle.RxBusyState == USART_BUSY_IN_RX) {
		if (USARTHandle.RxLen > 0) {
			...
			if (USARTHandle.USART_Config.USART_WordLength == USART_WORDLEN_9BITS) {
				...
				if (USARTHandle.USART_Config.USART_ParityControl
					...
					//read only first 9 bits so mask the DR with 0x01FF
					*((uint16_t*) USARTHandle.pRxBuffer) =GET_KBITS(USARTx_ptr(USARTHandle.USARTx)->DR,9,0);//& (uint16_t) 0x01FF);
					...
					*((uint16_t*) USARTHandle.pRxBuffer) =
							GET_KBITS(USARTx_ptr(USARTHandle.USARTx)->DR,8,0);//USARTHandle.pRxBuffer++;
					USARTHandle.RxLen -= 1;
				}
			} else {
				//We are going to receive 8bit data in a frame
				...
				if (USARTHandle.USART_Config.USART_ParityControl
					...
					//read 8 bits from DR
					*((uint16_t*) USARTHandle.pRxBuffer) =
							(uint8_t) GET_KBITS(USARTx_ptr(USARTHandle.USARTx)->DR,8,0);//& (uint8_t) 0xFF);
					...
					//read only 7 bits , hence mask the DR with 0X7F
					*((uint16_t*) USARTHandle.pRxBuffer) =
							(uint8_t) GET_KBITS(USARTx_ptr(USARTHandle.USARTx)->DR,7,0);//& (uint8_t) 0x7F);
		...
		if (!USARTHandle.RxLen) {
			//disable the rxne
			CLR_BIT_N( USARTx_ptr(USARTHandle.USARTx)->CR1,USART_CR1_RXNEIE);
 ...
 void USART_TC_EV(USART_Handle_t USARTHandle) {
	//close transmission and call application callback if TxLen is zero
	if (USARTHandle.TxBusyState == USART_BUSY_IN_TX) {
		//Check the TxLen . If it is zero then close the data transmission
		if (!USARTHandle.TxLen) {
			//Implement the code to clear the TC flag
			CLR_BIT_N( USARTx_ptr(USARTHandle.USARTx)->SR, USART_SR_TC);
 ...
 void USART_TXE_EV(USART_Handle_t USARTHandle) {
	uint16_t *pdata;
	if (USARTHandle.TxBusyState == USART_BUSY_IN_TX) {
		...
		if (USARTHandle.TxLen > 0) {
			...
			if (USARTHandle.USART_Config.USART_WordLength == USART_WORDLEN_9BITS) {
				...
				USARTx_ptr(USARTHandle.USARTx)->DR =GET_KBITS(*pdata,9,0);
				...
				//This is 8bit data transfer
				USARTx_ptr(USARTHandle.USARTx)->DR = GET_KBITS(*USARTHandle.pTxBuffer,8,0);//& (uint8_t) 0xFF);
		...
		if (USARTHandle.TxLen == 0) {
			...
			CLR_BIT_N(USARTx_ptr(USARTHandle.USARTx)->CR1, USART_CR1_TXEIE);
		}
	}

}
 void USART_LBD_EV(USART_Handle_t USARTHandle) {

}
 void USART_CTS_EV(USART_Handle_t USARTHandle) {
	//Implement the code to clear the CTS flag in SR
	CLR_BIT_N(USARTx_ptr(USARTHandle.USARTx)->SR,USART_SR_CTS);
